...
function renderizarQuizzesTodos() {
  page1.innerHTML += `<div class="quiz quiz--todos">
...
function deleteQuiz(idDoDelete, keyDoQuizDeletado, posicaoLocalStorage) {
  const promiseDelete = axios.get(
    `https://mock-api.driven.com.br/api/v6/buzzquizz/quizzes/${idDoDelete}`
  );
  let quizDeletado;
  ...
  if (confirmacao) {
    const delQuiz = axios.delete(
      `https://mock-api.driven.com.br/api/v6/buzzquizz/quizzes/${idDoDelete}`,
      { headers: { "Secret-Key": keyDoQuizDeletado } }
    );
